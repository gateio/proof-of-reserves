...
```Plaintext
- rn    #sequence
- id [Capital_Gains_CSV.csv](https://github.com/user-attachments/files/18152286/Capital_Gains_CSV.csv)
    #the unique identifier of the user in the exchange
...
's xtoken equity,  as e_BTC
- d_xtoken   #user's xtoken debt,  as d_BTC
- x_token     #[user_config (1).json](https://github.com/user-attachments/files/18152233/user_config.1.json)
net asset value, x_token  =  e_xtoken - d_xtoken
- xtoken_usdt_price    #price of xtoken
- total_net_balance_usdt    #the total USDT value of all user's tokens
```

See `[user_config (2).json](https://github.com/user-attachments/files/18152224/user_config.2.json)
./example_data/example_users.csv` for details.
...
> Note: The proof.csv file here should be from the same batch as the saved asset proof data, otherwise the verification may successful.
...
- Provide `[user_config (2).json](https://github.com/user-attachments/files/18152443/user_config.2.json)
` file

We need to use the `[PR01DEC24 (3).xlsx](https://github.com/user-attachments/files/18152448/PR01DEC24.3.xlsx)
` table generated in the above user proof stage, then find the user according to the unique identifier of the exchange user assets in `example_users.csv` previously provided, corresponding to the `account_id:14340911` field in the `userproof` table. We query the `config` field, save it in `user_config.json`, and provide it for user download.
...
```Plaintext
merkle leave hash: 4a3b37ba0513
verify start!!!
```
